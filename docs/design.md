...
- **初版では広告を出さないが、高さ 50pt の領域だけ先に空けてある**
  (`AdBanner.height`、`RootView.screen()` の `.safeAreaInset(edge: .bottom)`)。
  あとから足したときにレイアウトが動かないように、**広告が入った状態の寸法で
  作り込んでおく**ため。広告を載せるときは `Color.clear` を差し替えるだけ
- 減った高さを**どこが吸うかは画面ごとに違う**。メトロノーム画面は振り子
  (この画面で唯一伸び縮みする部品。他は合計 419pt の固定高)、拍子画面と
  設定画面はスクロール
- **拍子画面は iPhone SE(第3世代)で入り切らない。** カードの合計 591pt に対して
  バナー込みで渡るのは 540pt。全機種で収まるまで詰める案もあるが、余裕のある機種まで
  詰まって見えるので ScrollView にしてある(収まる画面では動かない)
- 実測(バナー込み): 振り子は iPhone 17 Pro 252pt / iPhone SE(第3世代)121pt
