...
- 高さは **86pt**。バナー 50 + 載せる面の内側余白 12×2 + タブバーとの間 12。
  デザイン(参照実装の `AdBar`)がバナーをそのまま置かず、ガラスの面に載せて
  タブバーと離しているので、その余白まで含めて空ける
...
- 実測(領域 86pt 込み): 振り子は iPhone 17 Pro 216pt / iPhone SE(第3世代)85pt
